#!/usr/bin/env python
# Purpose: Create a .csv file of most freq. network contributing to a given network's AI value. 
# Input: Kong2019 MSHBM parc output and network contributions rh_labels and lh_labels values
# Output: 1 .csv file with most frequent network contributor
#
...
# Python version: 3.6 (`ml python/3.8`)
...
with open('/fslgroup/fslg_spec_networks/compute/code/HCPD_analysis/ai_spec/REST/network_level_ai/subjids.txt', 'r') as f:
    # Read the lines of the file and store them in a list
    subject_ids = f.read().splitlines()

# Loop through subject IDs to finding avg. AI within network for each of 17 network matrices
for seed_network in range(1,18):
	sub_count=0
	count=0
	for sub1 in subject_ids:
		...
		for network in range(0,18): #Range must be set to actual number of networks +1
			...
			test_dir: str="/fslgroup/fslg_HBN_preproc/compute/HCPD_analysis/parc_output_fs6_HCPD_REST/generate_individual_parcellations/ind_parcellation/test_set"
			...
			# path to network_level_ai output
			sec_dir: str="/fslgroup/fslg_csf_autism/compute/HCPD_analysis/parc_output_fs6_HCPD_REST/quant_metrics/network_level_ai"
			sec_name = "sub-" + sub_name + "_NETWORK-" + str(seed_network) + "_AI_FS6.mat"
	...
	r_name = "MSHBM2019_LONG_NETWORK_LEVEL_AI_NETWORK-" + str(seed_network) + "_HCPD_230421.csv" #file name	
